...
public class GameAppExt
{
    public static void main(String[] args) {
        ...
        while (true) { // Ye loop tab tak chalegi jab tak user 3 press kar ke exit naa kar le. So basically agar koi player
            // haar bhi jaaye we will have to keep him inside this loop.
            ...
            if (option ==2) {
                ...
                while (true) // This loop keeps running until the player is alive. Once he loses, this loop will terminate
                {
                    ...
                    Monster opponent = Game.getOpponent(selected_node);// Finding the monster in the present location
                    mysidekick = Game.Get_Best_Sidekick(); // Calling the function to get the sidekick with most current XP
                    ...
                    if (option1 == 1 || option1 == 2 || option1 == 3 || option1 == 4){ // matlab ki user ne apna location choose kar liya hai.
                        ...
                        while (true) { // this loop will regualte the results of the game after each round/ will terminate when one round is finished
                            if (Sidekick.getStatus().equals("Active")) {
                                System.out.println("Type yes if you want to use a sidekick, else type no");
                                String choice1 = input.next();
                                mysidekick = Game.Get_Best_Sidekick();
                                if (choice1.equals("yes")) {
                                    sidekickused++;
                                    if (mysidekick.equals(tempminion)) {
                                        System.out.println("You have a sidekick " + mysidekick.getHerotype() + " with you. Att" +
                                                "ack of sidekick is " + mysidekick.getAttacking_Power());
                                        if (cloningusedonce == 1) {
                                            System.out.println("Press c to use cloning ability. Else press f to move to the fight");
                                            String choice2 = input.next();
                                            if (choice2.equals("c")) {
                                                System.out.println("Cloning done");
                                                cloning++; // This will tell if cloning is chosen by the user in the current fight or not.
                                                cloningusedonce++; // This will ensure that user can use cloning only once during his lifetime.
                                            }
                                            else {
                            ...
                            while (user.getHP() > 0 && opponent.getHP() > 0) { // This loop will end when one of the fighters looses.
                                if (counter < 4) {
                                    ...
                                    if (choice == 1) {
                                        ...
                                        user.Use_Normal_Power(opponent);
                                        user.Attack(opponent);
                                            if (sidekickused == 1) {
                                                if (mysidekick.getHP() > 0) {
                                                    if (mysidekick.equals(tempminion)) {
                                                        if (cloning > 0) {
                                                            copy1 = (Minion) mysidekick.ReturnClone();
                                                            copy2 = (Minion) mysidekick.ReturnClone();
                                                            copy3 = (Minion) mysidekick.ReturnClone();
                                                            mysidekick.Attack(opponent);
                                                            copy1.Attack(opponent);
                                                            copy2.Attack(opponent);
                                                            copy3.Attack(opponent);
                                                            opponent.Attack_After_Attack(user, mysidekick, copy1, copy2, copy3);
                                                        } else {
                                                            mysidekick.Attack(opponent);
                                                            opponent.Attack_After_Attack(user, mysidekick);
                                                        }
                                    ...
                                    if (choice == 2) {
                                        ...
                                        if (sidekickused == 1) {
                                            if (mysidekick.getHP() > 0) {
                                                if (mysidekick.equals(tempknight)) {
                                                    if (opponent.getLevel() == 2) {
                                                        user.Use_Normal_Power(opponent);
                                                        user.setDefending_Power(user.getDefending_Power() + 5);
                                                        opponent.Attack_After_Defense(user, mysidekick);
                                                    } else {
                                                        user.Use_Normal_Power(opponent);
                                                        opponent.Attack_After_Defense(user,mysidekick);
                                                    }
                                                    ...
                                                    if (cloning > 0) {
                                                        ...
                                                        copy3 = (Minion) mysidekick.ReturnClone();
                                                        user.Use_Normal_Power(opponent);
                                                        opponent.Attack_After_Defense(user,mysidekick,copy1,copy2,copy3);
                                                    }
                                                    else{
                                                        user.Use_Normal_Power(opponent);
                                                        opponent.Attack_After_Defense(user,mysidekick);
                                            ...
                                            else{ // Agar sidekick dead ho gaya to hame sirf user ko affect karna hai
                                                user.Use_Normal_Power(opponent);
                                    ...
                                    System.out.println("1) Attack");
                                    System.out.println("2) Defense");
                                    System.out.println("3) Speacial Move");
                                    ...
                                    if (choice2 == 3) {
                                        ...
                                        if (user.getHerotype().equals("Warrior")) {
                                            ...
                                            while (u<3) {
                                                System.out.println("Choose move: ");
                                                System.out.println("1) Speacial Attack");
                                                System.out.println("2) Speacial Defense");
                                                int p = input.nextInt();
                                                if (p == 1)
                                                {
                                                    user.Attack(opponent);
                            ...
                            if (opponent.getHP() <= 0) {
                                if (goback == 0) {
                                    ...
                                    if (wins <= 3) {
                                        ...
                                        if (sidekickused == 1)
                                        {
                                            if (cloningusedonce == 2)
                                            {
                                                cloningusedonce = 0; // setting it to zero so that it can only be used once during a lifetime.
                                            }
                                            if (mysidekick.getisDead() == false) {
                                                mysidekick.setHP(100);
                                                mysidekick.setCurXP(mysidekick.getCurXP() + user.getXP() / 10); //Updating the XP of the sidekick after the match..
                                            }
                                            else{
                                                System.out.println("The sidekick you bought died in the game. Now you can not use it further.");
                                            }
                                        }
                                        System.out.println("If you would like to buy a sidekick, type yes. Else type no to upgrade level."); // Starting with the work of
                                        // sidekick type heroes.
                                        ...
                                        if (sk.equals("yes")) {
                                                ...
                                                if (num4 == 1) {
                                                    ...
                                                    Sidekick m1 = new Minion();
                                                    m1.UpdatePrice(num5);
                                                    Game.ListOfSK.add(m1);
                                                    Sidekick.setStatusActive();
                                                    System.out.println("You bought a sidekick: Minion");
                                                    System.out.println("XP of sidekick is 0.0");
                                                    System.out.println("Attack of sidekick is " + m1.getAttacking_Power());
                                                    // set the sold price of the minion to num5. only if it is greater than the base price.
                                                }
                                                if (num4 == 2) {
                                                    System.out.println("XP to spend: ");
                                                    long num6 = input.nextLong();
                                                    Sidekick m2 = new Knight();
                                                    m2.UpdatePrice(num6);
                                                    Game.ListOfSK.add(m2);
                                                    Sidekick.setStatusActive();
                                                    System.out.println("You bought a sidekick: Knight");
                                                    System.out.println("XP of sidekick is 0.0");
                                                    System.out.println("Attack of sidekick is " + m2.getAttacking_Power());
...
final class Game{
    ...
    public static ArrayList<Sidekick> ListOfSK = new ArrayList<Sidekick>();
    ...
    public static Sidekick Get_Best_Sidekick()
    {
        long max=0;
        Sidekick temp = new Sidekick();
        for (int l = 0; l < ListOfSK.size(); l++ )
        {
            if (ListOfSK.get(l).getCurXP() >= max && ListOfSK.get(l).getisDead() == false)
            {
                max = ListOfSK.get(l).getCurXP();
                temp = ListOfSK.get(l);
            }
        }
        return temp;
    ...
    {
        Sidekick temp = new Sidekick();
        if (ListOfSK.size() == 1 && ListOfSK.get(0).getisDead()== false)
        {
            temp = ListOfSK.get(0);
        }
        else if (ListOfSK.size() > 1) {
            for (int l = 0; l < ListOfSK.size() - 1; l++) {
                if (ListOfSK.get(l).compareTo(ListOfSK.get(l+1)) == 1 || ListOfSK.get(l).compareTo(ListOfSK.get(l+1)) == 0 && ListOfSK.get(l).getisDead() == false) {
                    temp = ListOfSK.get(l);
                }
            }
        }
...
class Hero
{
    ...
    protected final void setHerotype(String ht)
    {
        Herotype = ht;
    }
...
class Thief extends Hero
{
    ...
    public void Use_Speacial_Power(Monster monster) {
        ...
        if (this.getHP() + this.stolen_HP <= this.getMaxHP()) {
...
class Healer extends Hero
{
    ...
    public void Use_Speacial_Power(Monster monster) {
        ...
        if (this.getHP() + this.increased_HP <= this.getMaxHP()) {
...
class Sidekick extends Hero implements Comparable<Sidekick> {
    private long BaseXP;
    private long soldXP;
    private long curXP;
    ...
    private static String Status = "Not Active";// let us see if we need it
    public static void setStatusActive()
    {
        Status = "Active";
    }
    public void setCurXP(long cxp)
    {
        curXP = cxp;
    }
    public long getCurXP()
    {
        return curXP;
    }
    ...
    public void setSoldXP(long sxp)
    {
        soldXP = sxp;
    }
    public long getSoldXP()
    {
        return soldXP;
    }
    public void setBaseXP(long num)
    {
        BaseXP = num;
    }
    ...
    public void UpdateAttackingPower(long num)
    {
        this.setAttacking_Power(num + this.getAttacking_Power());
    }
    public void UpdatePrice(long num6)
    {
        this.setSoldXP(num6);
        long temp = num6 - this.getBaseXP();
        long increase = Math.round(temp*0.5);
        this.UpdateAttackingPower(increase);
    }
    @java.lang.Override
    public void Attack(Monster mons1)
    {
        if ((mons1.getHP() - this.getAttacking_Power()) >= 0) {
            mons1.setHP(mons1.getHP() - this.getAttacking_Power());
            System.out.println("Sidekick attacked and inflicted " + this.getAttacking_Power() + " damage to the monster.");
            System.out.println("Sidekick HP: " + this.getHP()+ "/" + this.getMaxHP());
        }
        else {
            mons1.setHP(0);
            System.out.println("Sidekick attacked and inflicted " + this.getAttacking_Power() + " damage to the monster.");
    ...
    protected Object ReturnClone(){
        return this;
    }
    protected String getCloned()
    {
        return "yes";
    }
    protected void setCloned(String l)
    {

    }
    @java.lang.Override
    public int compareTo(Sidekick sk)
    {
        if (this.getCurXP() == sk.getCurXP())
        {
           return 0;
        }
        else if(this.getCurXP() < sk.getCurXP())
        {
            return -1;
        }
        else {
    ...
    public boolean equals(Object object)
    {
        ...
        {
            Sidekick sk = (Sidekick) object;
            return (this.getHerotype().equals(sk.getHerotype()));
        }
        else
        {
            return false;
...
class Minion extends Sidekick implements Cloneable{
    private String Cloned = "No";
    @java.lang.Override
    public void setCloned(String l)
    {
        Cloned = l;
    }
    @java.lang.Override
    ...
    public Minion()
    {
        this.setBaseXP(5);
        this.setMaxHP(100);
        this.setAttacking_Power(1);
        this.setCurXP(0);
        this.setHerotype("Minion");
    }
    @java.lang.Override
    ...
    public boolean equals(Object object)
    {
        if (object != null && getClass() == object.getClass())
        {
            Minion m1 = (Minion) object;
            return (this.getHerotype().equals(m1.getHerotype()));
        }
        else{
            return false;
        }
...
class Knight extends Sidekick{
    ...
    public boolean equals(Object object)
    {
        if (object != null && getClass() == object.getClass())
        {
            Knight kn = (Knight) object;
            return (this.getHerotype().equals(kn.getHerotype()));
        }
        else{
            return false;
        }
...
class Monster
{
    ...
    public void Attack_After_Defense(Hero user, Sidekick sk)
    {
        Random rand1 = new Random();
        long temp =0;
        long mean = this.getHP()/8;
        ...
        if (num3- user.getDefending_Power() > 0) {
            ...
            if (sk.getHP() - (3 *(num3 - user.getDefending_Power())) / 2 >0) {
                sk.setHP(sk.getHP() - (3 * (num3 - user.getDefending_Power())) / 2);
                System.out.println("Sidekick's HP: " + sk.getHP() + "/" + sk.getMaxHP());
            }
            else {
                sk.setHP(0);
                sk.setisDeadTrue();
    ...
    public void Attack_After_Defense(Hero user, Sidekick sk, Minion m1, Minion m2, Minion m3)
    {
        Random rand1 = new Random();
        long temp =0;
        long mean = this.getHP()/8;
        ...
        if (num3- user.getDefending_Power() > 0) {
            ...
            if (m1.getHP() - (3 * (num3 - user.getDefending_Power())) / 2 >0){
                m1.setHP(m1.getHP() - (3 * (num3 - user.getDefending_Power()) ) / 2);
                System.out.println("Sidekick's HP: " + m1.getHP() + "/" + m1.getMaxHP());
            }
            else {
                m1.setHP(0);
                m1.setisDeadTrue();
            ...
            if (m2.getHP() - (3 * (num3 - user.getDefending_Power())) / 2 > 0){
                m2.setHP(m2.getHP() - (3 * (num3 - user.getDefending_Power())) / 2);
                System.out.println("Sidekick's HP: " + m2.getHP() + "/" + m2.getMaxHP());
            }
            else {
                m2.setHP(0);
                m2.setisDeadTrue();
            ...
            if (m3.getHP() - (3 * (num3 - user.getDefending_Power())) / 2 > 0){
                m3.setHP(m3.getHP() - (3 * (num3 - user.getDefending_Power())) / 2);
                System.out.println("Sidekick's HP: " + m3.getHP() + "/" + m3.getMaxHP());
            }
            else {
                m3.setHP(0);
                m3.setisDeadTrue();
    ...
    public void Attack_After_Attack(Hero user,  Sidekick sk)
    {
        Random rand1 = new Random();
        long temp = 0;
        long mean = Math.round(this.getHP() / 8);
        ...
        if (sk.getHP() - (3 * num3) / 2 >0) {
            sk.setHP(sk.getHP() - (3 * num3) / 2);
            System.out.println("Sidekick's HP: " + sk.getHP() + "/" + sk.getMaxHP());
        }
        else {
            sk.setHP(0);
    ...
    public void Attack_After_Attack(Hero user, Sidekick sk, Minion m1, Minion m2, Minion m3)
    {
        Random rand1 = new Random();
        long temp = 0;
        long mean = Math.round(this.getHP() / 8);
        ...
        if (m1.getHP() - (3 * num3) / 2 >0){
            m1.setHP(m1.getHP() - (3 * num3) / 2);
            System.out.println("Sidekick's HP: " + m1.getHP() + "/" + m1.getMaxHP());
        }
        else {
            m1.setHP(0);
        ...
        if (m2.getHP() - (3 * num3) / 2 > 0){
            m2.setHP(m2.getHP() - (3 * num3) / 2);
            System.out.println("Sidekick's HP: " + m2.getHP() + "/" + m2.getMaxHP());
        }
        else {
            m2.setHP(0);
        ...
        if (m3.getHP() - (3 * num3) / 2 > 0){
            m3.setHP(m3.getHP() - (3 * num3) / 2);
            System.out.println("Sidekick's HP: " + m3.getHP() + "/" + m3.getMaxHP());
        }
        else {
            m3.setHP(0);
...
class Lionfang extends Monster{
    ...
    public void Attack_After_Defense(Hero user, Sidekick sk)
    {
        ...
        if (n == 9)
        {
            ...
            System.out.println("The monster attacked and inflicted " + (temphp / 2 - user.getDefending_Power()) + " damage to you.");
            System.out.println("Your HP: " + user.getHP() + "/" + user.getMaxHP() + " Monster's HP: " + this.getHP() + "/" + this.getMaxHP());
            if (sk.getHP() - (3 * (temphp / 2 - user.getDefending_Power())) / 2 >0) {
                sk.setHP(sk.getHP() - (3 * (temphp / 2 - user.getDefending_Power())) / 2);
                System.out.println("Sidekick's HP: " + sk.getHP() + "/" + sk.getMaxHP());
            }
            else {
                sk.setHP(0);
            ...
            if (n<=8)
            {
                super.Attack_After_Defense(user,sk);
            }
        }
    }
    @java.lang.Override
    public void Attack_After_Defense(Hero user, Sidekick sk, Minion m1, Minion m2, Minion m3)
    {
        ...
        if (n == 9)
        {
            ...
            if (m1.getHP() - (3 * (temphp / 2 - user.getDefending_Power())) / 2 >0){
                m1.setHP(m1.getHP() - (3 * (temphp / 2 - user.getDefending_Power()) ) / 2);
                System.out.println("Sidekick's HP: " + m1.getHP() + "/" + m1.getMaxHP());
            }
            else {
                m1.setHP(0);
            ...
            if (m2.getHP() - (3 * (temphp / 2 - user.getDefending_Power())) / 2 >0)
            {
                m2.setHP(m2.getHP() - (3 * (temphp / 2 - user.getDefending_Power()) / 2));
                System.out.println("Sidekick's HP: " + m2.getHP() + "/" + m2.getMaxHP());
            }
            else {
                m2.setHP(0);
            ...
            if (m3.getHP() - (3 * (temphp / 2 - user.getDefending_Power())) / 2 > 0){
                m3.setHP(m3.getHP() -  (3 * (temphp / 2 - user.getDefending_Power()))/ 2);
                System.out.println("Sidekick's HP: " + m3.getHP() + "/" + m3.getMaxHP());
            }
            else {
                m3.setHP(0);
            ...
            if (n<=8)
            {
                super.Attack_After_Defense(user,sk, m1,m2,m3);
            }
        }
    }
    @java.lang.Override
    ...
    public void Attack_After_Attack(Hero user, Sidekick sk)
    {
        ...
        if(n==9)
        {
            ...
            System.out.println("The monster attacked and inflicted " + temphp/2 + " damage to you.");
            System.out.println("Your HP: " + user.getHP() + "/" + user.getMaxHP()+ " Monster's HP: " + this.getHP() + "/" + this.getMaxHP());
            if (sk.getHP() - (3 * (temphp/2)) / 2 >0) {
                sk.setHP(sk.getHP() - (3 * (temphp / 2)) / 2);
                System.out.println("Sidekick's HP: " + sk.getHP() + "/" + sk.getMaxHP());
            }
            else {
                sk.setHP(0);
            ...
            if(n<=8)
            {
                super.Attack_After_Attack(user,sk);
            }
        }
    }
    @java.lang.Override
    public void Attack_After_Attack(Hero user, Sidekick sk, Minion m1, Minion m2, Minion m3)
    {
        ...
        if(n==9)
        {
            ...
            if (m1.getHP() - (3 * (temphp / 2)) / 2 >0){
                m1.setHP(m1.getHP() - (3 * (temphp / 2) ) / 2);
                System.out.println("Sidekick's HP: " + m1.getHP() + "/" + m1.getMaxHP());
            }
            else {
                m1.setHP(0);
            ...
            if (m2.getHP() - (3 * (temphp / 2)) / 2 >0)
            {
                m2.setHP(m2.getHP() - (3 * (temphp / 2) / 2));
                System.out.println("Sidekick's HP: " + m2.getHP() + "/" + m2.getMaxHP());
            }
            else {
                m2.setHP(0);
            ...
            if (m3.getHP() - (3 * (temphp / 2)) / 2 > 0){
                m3.setHP(m3.getHP() -  (3 * (temphp / 2))/ 2);
                System.out.println("Sidekick's HP: " + m3.getHP() + "/" + m3.getMaxHP());
            }
            else {
                m3.setHP(0);
            ...
            if(n<=8)
            {
                super.Attack_After_Attack(user,sk, m1, m2, m3);
            }
        }
    }
}
